...
/**
 * Resolves a workflow by identifier (UUID, name, slug, path) or returns a formatted error response.
 */
export async function requireWorkflowResolved(
  workflowIdentifier: string,
  userId?: string,
): Promise<{ workflow: Workflow } | { error: ToolCallResponse }> {
  const workflow = await resolveWorkflow(workflowIdentifier, userId);
  if (!workflow) {
    return {
      error: createErrorResponse(
        `Workflow "${workflowIdentifier}" not found. You can specify a workflow UUID, exact name, or slug (e.g. "review-workflow" or "parent-workflow/child").`,
      ),
    };
  }
  return { workflow };
...
/**
 * Resolves a workflow and loads all its nodes and edges in parallel.
 */
export async function requireWorkflowGraphResolved(
  workflowIdentifier: string,
  userId?: string,
): Promise<
...
> {
  const wfCheck = await requireWorkflowResolved(workflowIdentifier, userId);
...
/**
 * Resolves both a workflow and a specific node within it by UUID, name, or slug.
 */
export async function requireNodeResolved(
  workflowIdentifier: string,
  nodeIdentifier: string,
  userId?: string,
): Promise<
...
> {
  const graphCheck = await requireWorkflowGraphResolved(workflowIdentifier, userId);
  if ("error" in graphCheck) return graphCheck;

  const { workflow, nodes, edges } = graphCheck;
  const node = resolveNode(nodeIdentifier, nodes);
  if (!node) {
    return {
      error: createErrorResponse(
        `Node "${nodeIdentifier}" not found in workflow "${workflow.name}" (${workflow.id}). You can specify a node UUID, exact name, or slug.`,
      ),
    };
  }
